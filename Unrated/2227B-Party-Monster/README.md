...
$∗$A *substring* is a contiguous subsegment of a string. For example, `"acab"` is a substring of `"abacaba"` (it starts in position $3$ and ends in position $6$), but `"aa"` or `"d"` aren't substrings of this string. So the substring of the string $s$ from position $l$ to position $r$ is $s[l,r]=slsl+1…sr$.
...
The first line contains an integer $t$ ($1≤t≤104$) — the number of test cases. The descriptions of the test cases follow.
