...
| **Problem** | [2217C — Grid Covering](https://codeforces.com/contest/2217/problem/C) |
| **Rating** | 1300 |
...
The only line of each test case contains four integers `n, m, a, b` (`1 ≤ n, m, a, b≤ 10^9`).
...
In the second test case: `n=2`, `m=2`, `a=1`, `b=1`. One possible way to cover the grid is if Prakul starts by moving **down**: 
 ...
 - Start at `(1, 1)`.
- Jump down `a=1` tile: `(1, 1) → (2, 1)`.
- Jump right `b=1` tile: `(2, 1) → (2, 2)`.
- Jump down `a=1` tile: `(2, 2) → (1, 2)`.
- Jump right `b=1` tile: `(1, 2) → (1, 1)`.

 The sequence of visited tiles is `{(1, 1), (2, 1), (2, 2), (1, 2)}`. In this case, all `2 · 2 = 4` tiles are covered, so the answer would be "YES". *Notice that Prakul wraps around the grid in the last two moves.*In the third test case: `n=4`, `m=2`, `a=2`, `b=1`. If Prakul starts by moving **down**, the moves look like: 

 - Start at `(1, 1)`.
- Jump down `a=2` tiles: `(1, 1) → (3, 1)`.
- Jump right `b=1` tile: `(3, 1) → (3, 2)`.
- Jump down `a=2` tiles: `(3, 2) → (1, 2)`.
- Jump right `b=1` tile: `(1, 2) → (1, 1)`.
- `·s`
...
> 🔗 [View on Codeforces](https://codeforces.com/contest/2217/problem/C)
